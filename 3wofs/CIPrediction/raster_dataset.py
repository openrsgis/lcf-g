...
def get_feature_vertices_data():
    with open('data/adjacencies.pkl.pkl', 'rb') as f:
...
class LoadData(Dataset):
    ...
    @staticmethod
    def get_data(labels, data, idx,max_nodes=512):
        """
        ...
        feature_vertice = np.array(feature_vertices[idx])


        feature_vertice=feature_vertices[idx][:,-4:]
        data_x = np.pad(feature_vertice,
                        ((0,max_nodes-feature_vertice.shape[0]),(0,0)),
    ...
    def normalize_base(data, norm_dim, train_mode):
        """
        Compute the normalized basis using logarithmic transformation.

        :param data: list of np.array, raw vertex feature data.
    ...
    def normalize_data(max_data, min_data, data):
        """
        Calculate normalized traffic data using the log transformation method.

        :param max_data: np.array, max data for each feature.
        :param min_data: np.array, min data for each feature.
        :param data: list of np.array, original vertex feature data without normalization.
        :return:
            list of np.array, normalized vertex feature data.
        """
        # Find the feature dimensions where max_data and min_data are equal
        constant_features = np.where(max_data == min_data)[0]

        normalized_data = []  # List to store the normalized data
        ...
        for i in range(len(data)):
            ...
            log_data_i = np.log1p(data_i)  # Use log1p to avoid log(0)

            # Apply Min-Max normalization formula
            normalized_i = (log_data_i - min_data) / (max_data - min_data)

            # For constant_features, set their normalized values to 1
            normalized_i[:, constant_features] = 1

            # Append the normalized data to the list
            normalized_data.append(normalized_i)
